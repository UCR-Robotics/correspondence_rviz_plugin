...
PointCloudCorrespondenceDisplay::PointCloudCorrespondenceDisplay() {
  start_color_property_ =
      new ColorProperty("Line Color", QColor(0x66, 0xcc, 0xff), "Color of the line", this, SLOT(updateStyle()), this);
  ...
  max_length_property_ =
      new FloatProperty("Max Line Length", 0.0,
...
void PointCloudCorrespondenceDisplay::processMessage(
    const correspondence_rviz_plugin::PointCloudCorrespondenceConstPtr& msg) {
  if (msg->index_source.size() != msg->index_target.size()) {
    setStatus(StatusProperty::Error, "Unmatched Index!",
              "The size of the correspondence index of two point clouds are not match.");
    return;
  }
...
std::vector<Ogre::Vector3> PointCloudCorrespondenceDisplay::transform_pointcloud(const sensor_msgs::PointCloud2& cloud,
